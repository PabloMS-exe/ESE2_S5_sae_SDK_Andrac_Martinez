...
# Création et connexion à l’instrument S2VNA via adresse IP et port
S2VNA = ARV_S2VNA("127.0.0.1", 5025, "ARV")
S2VNA.connect()

# Choix du paramètre S à mesurer (ex : S11, S21, etc.)
param_S = input("Choisissez une des paramètre S  (S11;S21;S12;S22) : ")
S2VNA.set_parametre_S(param_S)
mesure_S2VNA = Mesure_ARV(S2VNA)

# 2. Sauvegarde des mesures sur l’instrument sous forme de fichier CSV
...
# 3. Création des gabarits pour définir les zones de tolérance
...
# 4. Tracé de la courbe mesurée avec les gabarits sur le graphe
...
tracer.tracer()  # Affiche la courbe avec les limites du gabarit

# 5. Création du rapport PDF de mesure
pdf = Creation_PDF()
pdf.ajouter_page()

# Ajout du nom du technicien et de la date du test
nom_technicien = input("Entrez le nom du technicien : ")
pdf.set_xy(10, 10)
pdf.cell(100, 10, f"Le technicien est : {nom_technicien}", ln=0)
pdf.set_xy(150, 10)
pdf.cell(50, 10, f"Date : {datetime.today().strftime('%d/%m/%Y')}", align='R')

pdf.ln(10)  # Saut de ligne vertical
pdf.multi_cell(0, 10, "Rapport de Mesures Hyperfréquences :\n------------------------", align='C')

# Ajout des infos des gabarits utilisés dans le rapport
pdf.ajouter_texte(
...
# 6. Vérification de la conformité de la mesure au gabarit
...
# 7. Ajout des résultats mesure dans le PDF
pdf.ajouter_page()
...
# 8. Génération du fichier PDF final
pdf.generer("E:/BUT_GE2I/SDK_SAE/test_rapports/rapports_SAE_2.pdf")
